...
from core.sync_integration import create_sync_callback, initialize_sync_system
...
def main():
    ...
    sync_callback = create_sync_callback(
        log_results=True, alert_on_failure=True, custom_handler=custom_sync_callback
    )
    ...
    sync_manager = initialize_sync_system(
        ...
        custom_intervals={"consistency": 300, "incremental": 60},  # 5 minutes  # 1 minute
    ...
    for job_id, status in job_status.items():
        print(
            f"   - {job_id}: {status['job_type']} (enabled: {status['enabled']}, interval: {status['interval_seconds']}s)"
        )
