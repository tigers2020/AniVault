...
from core.sync_profiler import (
    SyncProfiler, ProfilerEvent, get_sync_profiler, 
    profile_operation, profile_sync_operation
)
...
def run_performance_analysis():
    ...
    for benchmark_name, result in benchmark_results.items():
        print(f"  {benchmark_name}: {result.throughput_per_sec:.2f} ops/sec, "
        ...
        if benchmark_name in performance_report["performance_targets"]:
            ...
            if "max_duration_ms" in target:
                if result.avg_duration_ms <= target["max_duration_ms"]:
                    targets_met += 1
                    print(f"✅ {benchmark_name}: Duration target met ({result.avg_duration_ms:.2f}ms <= {target['max_duration_ms']}ms)")
                else:
                    print(f"❌ {benchmark_name}: Duration target missed ({result.avg_duration_ms:.2f}ms > {target['max_duration_ms']}ms)")
            ...
            if "min_throughput_per_sec" in target:
                if result.throughput_per_sec >= target["min_throughput_per_sec"]:
                    targets_met += 1
                    print(f"✅ {benchmark_name}: Throughput target met ({result.throughput_per_sec:.2f}/sec >= {target['min_throughput_per_sec']}/sec)")
                else:
                    print(f"❌ {benchmark_name}: Throughput target missed ({result.throughput_per_sec:.2f}/sec < {target['min_throughput_per_sec']}/sec)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"- Top bottleneck: {results['bottlenecks'][0]['operation_name'] if results['bottlenecks'] else 'None'}")
