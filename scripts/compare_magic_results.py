...
def compare_results():
    """리팩토링 전후 비교."""
    # Before
    with open("magic_violations_phase2.json", 'r', encoding='utf-8') as f:
        before = json.load(f)
    
    # After
    with open("magic_violations_post_refactor.json", 'r', encoding='utf-8') as f:
        after = json.load(f)
    ...
    for file, reduced_count in sorted_improvements[:10]:
        ...
        print(f"  -{reduced_count:3d} ({pct:5.1f}%)  {file_short:50s}  ({before_count} → {after_count})")
