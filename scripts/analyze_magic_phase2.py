...
def analyze_violations(json_file: str):
    ...
    print("\n" + "=" * 80)
    print("🎯 **컨텍스트별 분포**\n")
    for context, count in sorted(context_violations.items(), key=lambda x: x[1], reverse=True):
        print(f"  {count:3d}개  {context}")
    
    # 타입별 분포
    print("\n" + "=" * 80)
    print("🔢 **타입별 분포**\n")
    for vtype, count in sorted(type_violations.items(), key=lambda x: x[1], reverse=True):
