...
with open("error_violations.json", encoding="utf-8") as f:
    data = json.load(f)

violations = data["violations"]

# Stage 1 대상 파일들
stage1_files = ["settings.py", "encryption.py", "tmdb_matching_worker.py"]
...
for filename in stage1_files:
    file_violations = [
        v for v in violations if filename in v["file"] and v["severity"] == "high"
    ]
...
file_counter = Counter(
    ...
    if not any(f in v["file"] for f in stage1_files)
)
print("우선순위 파일 TOP 5:")
