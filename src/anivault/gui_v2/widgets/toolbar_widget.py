...
class ToolbarWidget(QWidget):
    ...
    def set_view(self, view_name: str) -> None:
        ...
        if view_name == "groups":
            self._setup_groups_toolbar()
        elif view_name == "tmdb":
            self._setup_tmdb_toolbar()
        elif view_name == "organize":
            self._setup_organize_toolbar()
    ...
    def _setup_groups_toolbar(self) -> None:
        """Set up toolbar for groups view."""
        search_box = QLineEdit()
        search_box.setPlaceholderText("그룹 또는 파일 검색...")
        ...
        hint = QLabel(f"현재 탭: <strong>그룹 관리</strong>")
