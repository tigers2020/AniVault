...
def run_organize_execute_flow(
    ...
    if getattr(options, "enhanced", False):
        plan = use_case.generate_enhanced_plan_from_metadata(
            files, destination=getattr(options, "destination", "Anime")
        )
    else:
        plan = use_case.generate_plan_from_metadata(
            files, settings=window.app_context.settings
        )
...
def _resolve_organize_directory(window: MainWindow) -> Path | None:
    ...
    if (
        window.app_context.settings.folders
        and window.app_context.settings.folders.source_folder
    ):
