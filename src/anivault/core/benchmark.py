...
class BenchmarkRunner:
    ...
    def load_ground_truth(self, dataset_path: Path | str) -> None:
        ...
        logger.info("Loaded %d ground truth entries from %s", 
                   len(self.ground_truth), dataset_path)
    ...
    def create_sample_dataset(self, output_path: Path | str) -> None:
        ...
        logger.info("Created sample dataset with %d entries at %s", 
                   len(sample_entries), output_path)
    ...
    async def run_benchmark(self) -> Tuple[List[BenchmarkResult], BenchmarkSummary]:
        ...
        if not self.ground_truth:
            raise ValueError("No ground truth dataset loaded. Call load_ground_truth() first.")
        ...
        for i, entry in enumerate(self.ground_truth):
            logger.debug("Processing test case %d/%d: %s", 
                        i + 1, len(self.ground_truth), entry.filename)
            ...
            try:
                ...
                if match_result is None:
                    ...
                    is_correct = (
                        actual_tmdb_id == entry.expected_tmdb_id and
                        confidence_score >= entry.confidence_threshold
                    )
        ...
        logger.info("Benchmark completed: %d tests, %.1f%% accuracy, %.2fs total time",
                   len(results), summary.accuracy, total_time)
