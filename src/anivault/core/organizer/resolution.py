...
class ResolutionSummary:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.series_title:
            msg = "series_title cannot be empty"
            raise ValueError(msg)
...
class ResolutionAnalyzer:
    ...
    def analyze_series(
        ...
        Returns:
            Dictionary mapping series titles to ResolutionSummary objects
        ...
        series_file_counts: dict[str, int] = defaultdict(int)

        for info in resolution_infos:
            series_resolutions[info.series_title].add(info.is_high_res)
            series_file_counts[info.series_title] += 1

        # Create summaries
        summaries: dict[str, ResolutionSummary] = {}
        for series_title, res_types in series_resolutions.items():
            summaries[series_title] = ResolutionSummary(
                series_title=series_title,
                has_mixed_resolutions=len(res_types) > 1,
                resolutions=frozenset(res_types),
                file_count=series_file_counts[series_title],
            )

        self.logger.debug(
            "Resolution analysis: %d series, %d with mixed resolutions",
            len(summaries),
            sum(1 for s in summaries.values() if s.has_mixed_resolutions),
        )
