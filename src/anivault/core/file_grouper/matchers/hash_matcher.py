...
class HashSimilarityMatcher:
    ...
    def match(self, files: list[ScannedFile]) -> list[Group]:
        ...
        # Step 2: Group files by normalized hash
        hash_groups: dict[str, list[tuple[ScannedFile, str]]] = {}
        for file, original_title, normalized_title in file_titles:
            if normalized_title not in hash_groups:
                hash_groups[normalized_title] = []
            hash_groups[normalized_title].append((file, original_title))

        # Step 3: Convert to Group objects (use first original title as group name)
        result = []
        for normalized_hash, file_title_pairs in hash_groups.items():
