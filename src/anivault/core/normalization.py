...
def normalize_query_from_anitopy(anitopy_result: dict[str, Any]) -> dict[str, Any] | None:
...
def _detect_language(title: str) -> str:
    ...
    if japanese_ratio > 0.3:  # 30% threshold for Japanese
        return 'ja'
    elif korean_ratio > 0.3:  # 30% threshold for Korean
        return 'ko'
    elif latin_ratio > 0.5:  # 50% threshold for English
        return 'en'
    else:
        # If no clear majority, check for specific patterns
