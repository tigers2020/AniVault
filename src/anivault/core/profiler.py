...
class Profiler:
    ...
    def take_memory_snapshot(self, context: str = "manual") -> MemorySnapshot:
        ...
        logger.debug("Memory snapshot taken: %.2f MB current, %.2f MB peak, %d blocks",
                    current_mb, peak_mb, memory_blocks)
    ...
    def profile_section(self, section_name: str):
        ...
        try:
            ...
            logger.debug("Section '%s' completed in %.3fs, memory delta: %.2f MB",
                        section_name, duration, memory_delta)
    ...
    def print_summary(self, report: ProfilingReport) -> None:
        ...
        if report.memory_snapshots:
            ...
            for snapshot in report.memory_snapshots:
                print(f"{snapshot.timestamp}: {snapshot.current_memory_mb:.2f} MB "
    ...
    def export_detailed_analysis(self, report: ProfilingReport, output_path: Path | str) -> None:
        ...
        analysis = {
            ...
            "performance_analysis": {
                "throughput": {
                    "files_per_second": report.performance_metrics.total_files / max(report.duration_seconds, 1),
                    "matches_per_second": report.performance_metrics.successful_matches / max(report.duration_seconds, 1)
                },
                "accuracy": {
                    "success_rate": (report.performance_metrics.successful_matches / max(report.performance_metrics.total_files, 1)) * 100,
                    "high_confidence_rate": (report.performance_metrics.high_confidence_matches / max(report.performance_metrics.successful_matches, 1)) * 100
                },
                "efficiency": {
                    "cache_efficiency": report.performance_metrics.cache_hit_ratio,
                    "api_efficiency": ((report.performance_metrics.api_calls - report.performance_metrics.api_errors) / max(report.performance_metrics.api_calls, 1)) * 100
                }
            },
            "memory_analysis": {
    ...
    def _calculate_memory_growth(self, snapshots: List[MemorySnapshot]) -> Dict[str, float]:
        ...
        total_growth = last_snapshot.current_memory_mb - first_snapshot.current_memory_mb
