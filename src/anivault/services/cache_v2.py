...
class CacheEntry(BaseModel):
    ...
    cache_type: str = Field(..., description="Type of cache entry ('search' or 'details')")
...
class JSONCacheV2:
    ...
    def __init__(self, cache_dir: Path | str, statistics: StatisticsCollector | None = None) -> None:
    ...
    def _generate_file_path(self, key: str, cache_type: str = "search") -> Path:
        ...
        if cache_type == "search":
            ...
            raise ValueError(f"Invalid cache_type: {cache_type}. Must be 'search' or 'details'")
    ...
    def get(self, key: str, cache_type: str = "search") -> dict[str, Any] | None:
        ...
        try:
            ...
            # Verify key hash
            expected_hash = hashlib.sha256(key.lower().strip().encode("utf-8")).hexdigest()
    ...
    def clear(self, cache_type: str | None = None) -> int:
        ...
        try:
            if cache_type is None:
                ...
                cache_dir = self.search_dir if cache_type == "search" else self.details_dir
                for cache_file in cache_dir.glob("*.json"):
                    cache_file.unlink()
                    deleted_count += 1
            else:
    ...
    def get_cache_info(self, cache_type: str | None = None) -> dict[str, Any]:
        ...
        try:
            if cache_type is None:
                ...
                cache_dirs = [self.search_dir if cache_type == "search" else self.details_dir]
            else:
                raise ValueError(f"Invalid cache_type: {cache_type}")
