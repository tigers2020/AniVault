"""Build groups use case (Phase R3).

Builds series-based group display data from FileMetadata for GroupsView.
Full build and fast path (metadata-only update) are both handled here so that
grouping rules, tmdb_id merge, and fallback conditions live in one place.
"""
...
# Contract for group display dict consumed by GroupsView / GroupCard / DetailPanel.
# Kept as dict for this phase; can be promoted to TypedDict later.
GroupViewModel = dict[str, Any]
...
def _scanned_file_from_metadata(
    ...
    """Build a ScannedFile from FileMetadata for FileGrouper (groups-specific rules).

    Uses TitleExtractor + fallbacks; not shared with organize conversion.
    """
...
def _display_title_for_files(
    ...
    matched_with_title = [
        fm
        for fm in all_files
        if fm.tmdb_id is not None and fm.title and fm.title != fm.file_path.name
    ]
...
def _collect_tmdb_buckets(
    ...
    for series_name, file_list in series_groups.items():
        tmdb_ids: list[int] = [
            tid for fm in file_list if (tid := getattr(fm, "tmdb_id", None)) is not None
        ]
        if not tmdb_ids:
...
def _file_groups_to_series_groups(
    ...
    for group in file_groups:
        group_files_metadata = [
            files_by_path[resolved]
            for sf in group.files
            if (resolved := _path_resolved(sf.file_path)) in files_by_path
        ]
...
class BuildGroupsUseCase:
    """Use case for building group display data from FileMetadata."""
    ...
    def execute(
        ...
        """Build group display data from FileMetadata.

        Returns list of group dicts compatible with GroupsView (id, title, season,
        episodes, files, matched, confidence, resolution, language, file_metadata_list).
        """
        if not files:
            return []
        ...
        return [
            _build_group_dict(index, series_name, all_files)
            for index, (series_name, all_files) in enumerate(series_groups.items(), start=1)
        ]

    def apply_metadata_update(
        self,
        existing_groups: list[GroupViewModel],
        files: list[FileMetadata],
    ) -> list[GroupViewModel] | None:
        """Fast path: update group metadata when structure can be preserved.

        Only returns updated groups when we can safely keep the same group
        structure and only overwrite metadata (e.g. after TMDB manual apply).
        Otherwise returns None so the caller performs a full rebuild.

        Fallback (return None) when:
        - File path set differs from existing groups (added/removed paths).
        - Multiple groups would share the same tmdb_id after update (merge required).
        - Group dict contract cannot be preserved.
        """
        ...
        updated: list[GroupViewModel] = []
        for idx, group in enumerate(existing_groups, start=1):
            old_list = group.get("file_metadata_list", [])
            new_list = [
                files_by_path[_path_resolved(fm.file_path)]
                for fm in old_list
                if _path_resolved(fm.file_path) in files_by_path
            ]
            if not new_list:
                continue
