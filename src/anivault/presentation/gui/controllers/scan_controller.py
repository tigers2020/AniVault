...
from anivault.application.use_cases.scan_use_case import run_scan_in_process
...
class ScanController(BaseController):
    ...
    def _start_scan_process(self, directory: Path, extensions: list[str]) -> None:
        ...
        self.operation_progress.emit(OperationProgress(current=0, total=0, stage="scanning", message="스캔 시작"))
    ...
    def _poll_scan_queue(self) -> None:
        ...
        while True:
            ...
            if kind == ScanQueueMessageKind.PROGRESS:
                ...
                self._stop_poll_timer_only()
                self.operation_error.emit(OperationError(code="SCAN_FAILED", message="스캔 중 오류가 발생했습니다.", detail=payload))
                QTimer.singleShot(0, self._stop_scan_process)
                return
