...
def format_verify_result_for_json(
    ...
    if all_components_result is not None:
        ...
        any_failed = any(
            v.get("status") == VerificationStatus.FAILED.value
            for v in all_components_result.values()
            if isinstance(v, dict)
        )
        verify_results["verification_status"] = (
            VerificationStatus.FAILED.value if any_failed else VerificationStatus.SUCCESS.value
        )
