...
from anivault.infrastructure import TMDBClient
...
class CacheVerifier:
    ...
    def __init__(self, cache_db_path: Path | None = None) -> None:
        self._cache_db_path = cache_db_path or (get_project_root() / FileSystem.CACHE_DIRECTORY / TMDB_CACHE_DB)
...
class VerifyUseCase:
    ...
    def __init__(
        ...
        self._component_verifiers: Sequence[ComponentVerifier] = (
            tuple(component_verifiers) if component_verifiers is not None else (SettingsVerifier(), CacheVerifier())
        )
    ...
    async def verify_all(self) -> dict[str, VerificationEntry]:
        ...
        any_failed = any(entry.get("status") == VerificationStatus.FAILED.value for entry in results.values())
