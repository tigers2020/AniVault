...
class ParserWorker:
    ...
    def __init__(
        ...
        logger.info(f"Initialized ParserWorker with parse_function={getattr(parse_function, '__name__', 'unknown')}")
    ...
    def consume_queue(
        ...
        logger.info(f"ParserWorker finished: {self.stats['files_processed']} files processed")
        return results
...
class ParserWorkerPool:
    ...
    def start(self) -> None:
        ...
        if self._executor is None:
            ...
            self._workers = [
                ParserWorker(self.parse_function)
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if self._executor is not None:
            stats.update({
                "_threads": len(self._executor._threads) if hasattr(self._executor, '_threads') else None,
                "_work_queue_size": self._executor._work_queue.qsize() if hasattr(self._executor, '_work_queue') else None
            })
        ...
        if self._workers:
            ...
            stats.update({
                "total_files_processed": sum(ws["files_processed"] for ws in worker_stats),
                "total_parse_successes": sum(ws["parse_successes"] for ws in worker_stats),
                "total_parse_errors": sum(ws["parse_errors"] for ws in worker_stats),
                "total_queue_get_blocks": sum(ws["queue_get_blocks"] for ws in worker_stats),
                "total_queue_get_errors": sum(ws["queue_get_errors"] for ws in worker_stats),
