...
class ThreadExecutorManager:
    ...
    def __init__(self):
        ...
        self._tmdb_executor: Optional[ThreadPoolExecutor] = None
        self._file_scan_executor: Optional[ThreadPoolExecutor] = None
        self._general_executor: Optional[ThreadPoolExecutor] = None
        ...
        logger.info(f"ThreadExecutorManager initialized with workers: "
    ...
    def get_tmdb_executor(self) -> ThreadPoolExecutor:
        ...
        if self._tmdb_executor is None or self._tmdb_executor._shutdown:
            self._tmdb_executor = ThreadPoolExecutor(
                max_workers=self._tmdb_max_workers,
                thread_name_prefix="TMDBWorker"
            )
    ...
    def get_file_scan_executor(self) -> ThreadPoolExecutor:
        ...
        if self._file_scan_executor is None or self._file_scan_executor._shutdown:
            self._file_scan_executor = ThreadPoolExecutor(
                max_workers=self._file_scan_max_workers,
                thread_name_prefix="FileScanWorker"
            )
            logger.info(f"Created FileScan ThreadPoolExecutor with {self._file_scan_max_workers} workers")
    ...
    def get_general_executor(self) -> ThreadPoolExecutor:
        ...
        if self._general_executor is None or self._general_executor._shutdown:
            self._general_executor = ThreadPoolExecutor(
                max_workers=self._general_max_workers,
                thread_name_prefix="GeneralWorker"
            )
            logger.info(f"Created General ThreadPoolExecutor with {self._general_max_workers} workers")
    ...
    def get_configuration_info(self) -> dict:
        ...
        return {
            ...
            "tmdb_executor_active": (self._tmdb_executor is not None and 
                                   not self._tmdb_executor._shutdown),
            "file_scan_executor_active": (self._file_scan_executor is not None and 
                                        not self._file_scan_executor._shutdown),
            "general_executor_active": (self._general_executor is not None and 
                                      not self._general_executor._shutdown),
        }


# Global instance for easy access throughout the application
_thread_executor_manager: Optional[ThreadExecutorManager] = None
