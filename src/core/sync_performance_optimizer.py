...
from .sync_profiler import ProfilerEvent, get_sync_profiler
        ...
        try:
            ...
            if hasattr(self.db_manager, "engine") and self.db_manager.engine:
                ...
                return OptimizationResult(
                    ...
                    before_metrics={
                        "connection_time_ms": self.original_pool_settings.get(
                            "connection_time_ms", 100
                        )
                    },
                    after_metrics={"connection_time_ms": test_duration},
                    improvement_percent=(
                        (self.original_pool_settings.get("connection_time_ms", 100) - test_duration)
                        / self.original_pool_settings.get("connection_time_ms", 100)
                    )
                    * 100,
                    success=True,
                )
...
class BatchOptimizer:
    ...
    def optimize_batch_size(self, operation_type: str, test_data: list[Any]) -> OptimizationResult:
        ...
        try:
            ...
            logger.info(
                f"Optimal batch size for {operation_type}: {optimal_size} "
            ...
            return OptimizationResult(
                ...
                before_metrics={
                    "batch_size": batch_sizes_to_test[0],
                    "throughput": results[batch_sizes_to_test[0]]["throughput_per_sec"],
                },
                after_metrics={
                    "batch_size": optimal_size,
                    "throughput": results[optimal_size]["throughput_per_sec"],
                },
                improvement_percent=(
                    (
                        results[optimal_size]["throughput_per_sec"]
                        - results[batch_sizes_to_test[0]]["throughput_per_sec"]
                    )
                    / results[batch_sizes_to_test[0]]["throughput_per_sec"]
                )
                * 100,
            ...
            elapsed = time.time() - start_time
            logger.debug(
                f"Batch size optimization for {operation_type} completed in {elapsed:.3f}s"
            )
...
class SyncPerformanceOptimizer:
    ...
    def analyze_and_optimize(self) -> list[OptimizationResult]:
        ...
        if successful_optimizations:
            ...
            for result in successful_optimizations:
                logger.info(
                    f"  {result.optimization_name}: {result.improvement_percent:.1f}% improvement"
                )
        else:
    ...
    def _should_optimize_batch_sizes(self, bottlenecks) -> bool:
        ...
        for bottleneck in bottlenecks:
            if (
                bottleneck.event_type.value
                in ["db_bulk_insert", "db_bulk_update", "db_bulk_upsert"]
                and bottleneck.throughput_per_sec < 1000
            ):
                return True
        return False
    ...
    def get_optimization_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "average_improvement_percent": (
                sum(r.improvement_percent for r in successful_results) / len(successful_results)
