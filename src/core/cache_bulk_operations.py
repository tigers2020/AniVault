...
class CacheBulkOperations:
    ...
    def __init__(self, cache_core: CacheCore, db_integration: CacheDatabaseIntegration):
            ...
            for i in range(0, len(parsed_files), batch_size):
                ...
                for parsed_file in batch:
                    try:
                        ...
                        # Store in database
                        existing = session.query(ParsedAnimeInfo).filter_by(file_path=parsed_file.file_path).first()
                        if existing:
                            # Update existing record
    ...
    def bulk_update_tmdb_metadata(self, session: Session, updates: list[dict]) -> int:
        ...
        operation_id = sync_monitor.start_operation(
            SyncOperationType.BULK_UPDATE_TMDB,
            len(updates),
            cache_name=self.cache_core.cache_name
        )
        ...
        try:
            ...
            for update_data in updates:
                try:
                    ...
                    # Update database
                    parsed_file = session.query(ParsedAnimeInfo).filter_by(file_path=file_path).first()
                    if parsed_file:
                        for field, value in update_data.items():
    ...
    def bulk_update_tmdb_metadata_by_ids(
        ...
        try:
            ...
            logger.info(f"Bulk updated {updated_count} TMDB anime entries by IDs with {errors} errors")
            return updated_count
