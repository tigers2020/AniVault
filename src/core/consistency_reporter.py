...
from .consistency_validator import ConflictSeverity, ConflictType, DataConflict
...
class ConsistencyReporter:
    ...
    def get_report(self, report_id: int) -> Optional[ConsistencyReport]:
    ...
    def get_reports_by_job(self, job_id: str) -> List[ConsistencyReport]:
        ...
        """
        with self.db_manager.get_session() as session:
            return session.query(ConsistencyReport).filter(
                ConsistencyReport.job_id == job_id
            ).order_by(ConsistencyReport.started_at.desc()).all()
    ...
    def get_recent_reports(self, limit: int = 10) -> List[ConsistencyReport]:
        ...
        """
        with self.db_manager.get_session() as session:
            return session.query(ConsistencyReport).order_by(
                ConsistencyReport.started_at.desc()
            ).limit(limit).all()
    
    def get_report_summary(self, report_id: int) -> Optional[ReportSummary]:
    ...
    def generate_detailed_report(self, report_id: int) -> Dict[str, Any]:
        ...
        with self.db_manager.get_session() as session:
            conflicts = session.query(ConsistencyConflict).filter(
                ConsistencyConflict.report_id == report_id
            ).all()
        ...
        return {
            ...
            "conflicts": [
                {
                    ...
                    "database_data": json.loads(conflict.database_data) if conflict.database_data else None,
            ...
            } if report.error_message else None
        }
    ...
    def cleanup_old_reports(self, days_to_keep: int = 30) -> int:
        ...
        with self.db_manager.get_session() as session:
            ...
            old_conflicts = session.query(ConsistencyConflict).join(ConsistencyReport).filter(
                ConsistencyReport.created_at < cutoff_date
            ).all()
            ...
            # Delete old reports
            old_reports = session.query(ConsistencyReport).filter(
                ConsistencyReport.created_at < cutoff_date
            ).all()
