...
class SyncConfigManager:
    ...
    def get_config(self, config_name: str) -> SyncJobConfig | None:
    ...
    def create_custom_config(
        ...
        enabled: bool = True,
        trigger_types: list[SyncTrigger] | None = None,
        entity_types: list[SyncEntityType] | None = None,
        priority: int = 3,
        max_retries: int = 3,
        retry_delay_seconds: int = 30,
    ...
    def get_configs_by_type(self, job_type: SyncJobType) -> dict[str, SyncJobConfig]:
        ...
        """
        return {
            name: config for name, config in self.configs.items() if config.job_type == job_type
        }
    ...
    def get_configs_by_trigger(self, trigger: SyncTrigger) -> dict[str, SyncJobConfig]:
        ...
        """
        return {
            name: config for name, config in self.configs.items() if trigger in config.trigger_types
        }


# Global configuration manager instance
_global_config_manager: SyncConfigManager | None = None
...
def get_predefined_config(config_name: str) -> SyncJobConfig | None:
