...
class CacheIncrementalSync:
    ...
    def sync_all_entities_incremental(
        ...
        try:
            # Sync TMDB metadata
            tmdb_result = self.sync_tmdb_metadata_incremental(session, last_sync_timestamp, batch_size)
            
            # Sync parsed files
            parsed_result = self.sync_parsed_files_incremental(session, last_sync_timestamp, batch_size)
            ...
            combined_result = {
                ...
                "total_synced": tmdb_result.get("synced_count", 0) + parsed_result.get("synced_count", 0),
