...
from .reconciliation_strategies import (
...
class ConsistencyJob:
    ...
    def execute(self) -> dict[str, Any]:
        ...
        """
        if not self.enabled:
            return {"job_id": self.job_id, "status": "disabled", "message": "Job is disabled"}
        ...
        report = self.reporter.create_report(
            job_id=self.job_id, report_type="scheduled", started_at=start_time
        )
        ...
        try:
            ...
            if conflicts:
                logger.info(
                    f"Job {self.job_id}: Found {len(conflicts)} conflicts, starting reconciliation"
                )
                reconciliation_result = self.reconciliation_engine.reconcile_conflicts(
                    conflicts, self.strategy
                )
                ...
                self.reporter.update_report_with_resolution(
                    ...
                    (
                        "success"
                        if reconciliation_result and reconciliation_result.success
                        else "partial"
                    ),
                )
            else:
                ...
                self.reporter.update_report_with_resolution(report.id, [], self.strategy, "success")
            ...
            result = {
                ...
                "conflicts_resolved": (
                    reconciliation_result.conflicts_resolved if reconciliation_result else 0
                ),
                "conflicts_failed": (
                    reconciliation_result.conflicts_failed if reconciliation_result else 0
                ),
                "strategy_used": self.strategy.value,
                "reconciliation_success": (
                    reconciliation_result.success if reconciliation_result else True
                ),
                ...
                "reconciliation_errors": (
                    reconciliation_result.errors if reconciliation_result else []
                ),
            }
...
_global_scheduler: ConsistencyScheduler | None = None
