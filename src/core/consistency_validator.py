...
class ConsistencyValidator:
    ...
    def validate_all_consistency(self) -> list[DataConflict]:
        ...
        logger.info(
            f"Comprehensive validation completed: {len(all_conflicts)} total conflicts found"
        )
        return all_conflicts

    def _compare_anime_metadata_versions(
        self, db_anime: AnimeMetadata, cache_data: dict[str, Any], tmdb_id: int
    ) -> list[DataConflict]:
        ...
        if db_version != cache_version:
            severity = (
                ...
                if abs(db_version - cache_version) > 1
            ...
            conflicts.append(conflict)
            logger.debug(
                f"Version mismatch for anime {tmdb_id}: DB={db_version}, Cache={cache_version}"
            )
    ...
    def _compare_parsed_file_versions(
        self, db_file: ParsedFile, cache_data: dict[str, Any], file_id: int
    ) -> list[DataConflict]:
        ...
        if db_version != cache_version:
            ...
            conflicts.append(conflict)
            logger.debug(
                f"Version mismatch for file {file_id}: DB={db_version}, Cache={cache_version}"
            )
    ...
    def _extract_tmdb_id_from_cache_key(self, cache_key: str) -> int | None:
    ...
    def _extract_file_id_from_cache_key(self, cache_key: str) -> int | None:
