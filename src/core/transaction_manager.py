...
class TransactionManager:
            ...
            if nested:
                if not self._context_stack:
                    raise TransactionNestingError(
                        "Cannot create nested transaction without active parent"
                    )
                ...
                logger.debug(
                    f"Started nested transaction {context.id} (parent: {context.parent_id})"
                )
            else:
    ...
    def rollback(self, error: Exception | None = None) -> None:
    ...
    def transaction_scope(
        self, session: Session, nested: bool = False, timeout: int | None = None
    ...
    *,
    ...
    def decorator(f: F) -> F:
        def wrapper(*args, **kwargs):
            ...
            if (
                hasattr(session, "_transaction_manager")
                and session._transaction_manager is not None
            ):
            ...
            with tx_manager.transaction_scope(
                session, nested=current_nested, timeout=current_timeout
            ):
