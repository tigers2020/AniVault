...
class SyncStateManager:
    ...
    def get_last_sync_timestamp(self, entity_type: SyncEntityType) -> Optional[datetime]:
    ...
    def get_sync_state(self, entity_type: SyncEntityType) -> Optional[SyncState]:
...
class IncrementalSyncManager:
    ...
    def sync_tmdb_metadata_incremental(
        ...
        if not self.state_manager.lock_entity(entity_type):
            logger.warning(f"TMDB metadata sync already in progress, skipping")
        ...
        try:
            with sync_monitor.monitor_operation(
                ...
                if last_sync_timestamp is None or force_full_sync:
                    ...
                    query = session.query(AnimeMetadata).filter(
                        or_(
                    ...
                    logger.info(f"Performing incremental TMDB metadata sync since {last_sync_timestamp}")
                ...
                current_version = max((r.version for r in changed_records), default=last_sync_version)
    ...
    def sync_parsed_files_incremental(
        ...
        # Check if entity is already being synced
        if not self.state_manager.lock_entity(entity_type):
            logger.warning(f"Parsed files sync already in progress, skipping")
            return IncrementalSyncResult(
                entity_type=entity_type,
                records_found=0,
                records_processed=0,
        ...
        try:
            with sync_monitor.monitor_operation(
                ...
                if last_sync_timestamp is None or force_full_sync:
                    ...
                    # Incremental sync - get only changed records
                    query = session.query(ParsedFile).filter(
                        or_(
                    ...
                    logger.info(f"Performing incremental parsed files sync since {last_sync_timestamp}")
    ...
    def get_sync_status(self) -> Dict[SyncEntityType, Optional[SyncState]]:
