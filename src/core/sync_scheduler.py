...
from .consistency_scheduler import ConsistencyJob, ConsistencyScheduler
...
class SyncJob:
    ...
    def __init__(
        ...
        self.last_result: SyncJobResult | None = None
        self.run_count = 0
        self.error_count = 0
        self.retry_count = 0
    ...
    def execute(self, trigger: SyncTrigger = SyncTrigger.MANUAL) -> SyncJobResult:
        ...
        start_time = datetime.now(timezone.utc)
        logger.info(
            f"Starting sync job {self.config.job_id} (type: {self.config.job_type.value}, trigger: {trigger.value})"
        )
        ...
        try:
            ...
            logger.info(
                f"Sync job {self.config.job_id} completed with status: {result.status.value}"
            )
            return result
    ...
    def _execute_incremental_sync_job(
        self, trigger: SyncTrigger, start_time: datetime
    ) -> SyncJobResult:
        ...
        for entity_type in self.config.entity_types:
            try:
                with sync_monitor.operation(
                    operation_type=SyncOperationType.INCREMENTAL_SYNC, entity_type=entity_type.value
                ) as op:
    ...
    def _execute_full_sync_job(self, trigger: SyncTrigger, start_time: datetime) -> SyncJobResult:
        ...
        for entity_type in self.config.entity_types:
            try:
                with sync_monitor.operation(
                    operation_type=SyncOperationType.FULL_SYNC, entity_type=entity_type.value
                ) as op:
                    result = self.incremental_sync_manager.sync_entity_type(
                        entity_type, force_full_sync=True
                    )
                    sync_results.append(result)
                ...
                "last_result": (
                    {
                        ...
                        "duration_seconds": (
                            self.last_result.duration_seconds if self.last_result else None
                        ),
                        "records_processed": (
                            self.last_result.records_processed if self.last_result else None
                        ),
                        "error_message": (
                            self.last_result.error_message if self.last_result else None
                        ),
                    }
                    if self.last_result
                    else None
...
class SyncScheduler:
    ...
    def __init__(
        ...
        self.incremental_sync_manager = incremental_sync_manager or IncrementalSyncManager(
            self.metadata_cache
        )
        self.consistency_scheduler = consistency_scheduler or ConsistencyScheduler(
            self.metadata_cache
        )
    ...
    def add_job(self, config: SyncJobConfig) -> SyncJob:
        ...
        logger.info(
            f"Added sync job {config.job_id} (type: {config.job_type.value}, interval: {config.interval_seconds}s)"
        )
        return job
    ...
    def start_scheduler(self, check_interval: int = 30) -> None:
        ...
        self.scheduler_thread = threading.Thread(
            target=self._scheduler_loop, args=(check_interval,), daemon=True, name="SyncScheduler"
        )
...
_global_sync_scheduler: SyncScheduler | None = None
