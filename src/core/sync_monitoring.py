...
class SyncOperationMetrics:
    ...
    def complete(self, status: SyncOperationStatus, affected_records: int = 0, 
                 error_message: Optional[str] = None, **context):
...
class SyncMonitor:
    ...
    @contextmanager
    def monitor_operation(self, operation_type: SyncOperationType, 
                         cache_hit: bool = False, **context):
        """
    ...
    def _log_operation_success(self, metrics: SyncOperationMetrics) -> None:
        ...
        records_str = f"{metrics.affected_records} records" if metrics.affected_records > 0 else "no records"
    ...
    def log_cache_miss(self, key: str, operation_type: SyncOperationType) -> None:
        ...
        self.logger.debug(
            f"Cache miss: key={key} operation_type={operation_type.value}"
        )
    ...
    def log_cache_hit(self, key: str, operation_type: SyncOperationType) -> None:
        ...
        self.logger.debug(
            f"Cache hit: key={key} operation_type={operation_type.value}"
        )
    ...
    def log_bulk_operation_start(self, operation_type: SyncOperationType, 
                                record_count: int, **context) -> None:
    ...
    def log_bulk_operation_complete(self, operation_type: SyncOperationType, 
                                   record_count: int, duration_ms: float, 
                                   success_count: int, **context) -> None:
    ...
    def log_consistency_check(self, check_type: str, records_checked: int, 
                             inconsistencies_found: int, duration_ms: float) -> None:
    ...
    def log_reconciliation_event(self, event_type: str, records_affected: int, 
                                strategy: str, duration_ms: float) -> None:
