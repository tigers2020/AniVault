...
class CircuitBreakerConfiguration:
    ...
    def __init__(
        ...
        call_timeout: Optional[int] = 10,
        expected_exception: Optional[Set[Type[Exception]]] = None,
...
class DatabaseCircuitBreakerListener(CircuitBreakerListener):
    ...
    def on_failure(self, cb: CircuitBreaker, func: Callable, exc: Exception, *args, **kwargs) -> None:
    ...
    def on_timeout(self, cb: CircuitBreaker, func: Callable, exc: Exception, *args, **kwargs) -> None:
    ...
    def on_half_open(self, cb: CircuitBreaker) -> None:
        ...
        self.logger.warning(
            f"CircuitBreaker '{cb.name}' HALF-OPEN. Allowing a trial call."
        )
    
    def on_close(self, cb: CircuitBreaker) -> None:
        """Called when the circuit breaker closes."""
        self.logger.info(
            f"CircuitBreaker '{cb.name}' CLOSED. Database operations are now allowed."
        )
        # TODO: Trigger alerting here (Task 7)
...
class CircuitBreakerManager:
    ...
    def create_circuit_breaker(
        ...
        listener: Optional[DatabaseCircuitBreakerListener] = None,
    ) -> CircuitBreaker:
        """
        ...
        if config.name in self._circuit_breakers:
            logger.warning(f"Circuit breaker '{config.name}' already exists. Returning existing instance.")
        ...
        logger.info(f"Created circuit breaker '{config.name}' with configuration: {config.__dict__}")
...
def get_database_circuit_breaker(name: str = "database_circuit_breaker") -> Optional[CircuitBreaker]:
    """
...
def is_circuit_breaker_open(circuit_breaker: Optional[CircuitBreaker] = None) -> bool:
    """
