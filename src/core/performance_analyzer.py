...
from .sync_profiler import ProfilerEvent, SyncProfiler, get_sync_profiler
...
class PerformanceAnalyzer:
    ...
    def _analyze_single_bottleneck(
        self, bottleneck_data: dict[str, Any]
    ...
    def _calculate_severity_score(
        self, bottleneck_data: dict[str, Any], event_type: ProfilerEvent
    ) -> float:
    ...
    def _identify_root_causes(
        ...
        if event_type in [
            ...
            if throughput < 500:
                root_causes.append(
                    "Low bulk operation throughput - potential database indexing issues"
                )
        ...
        if event_type in [ProfilerEvent.CACHE_GET, ProfilerEvent.CACHE_SET]:
            if avg_duration > 10:
                root_causes.append(
                    "Slow cache operations - potential memory pressure or large object sizes"
                )
        ...
        if event_type == ProfilerEvent.DB_QUERY:
            if avg_duration > 50:
                root_causes.append(
                    "Slow database queries - missing indexes or inefficient query patterns"
                )
        ...
        if event_type == ProfilerEvent.SYNC_OPERATION:
            if avg_duration > 2000:
                root_causes.append(
                    "Slow synchronization - potential network or serialization bottlenecks"
                )
        ...
        if "memory_mb" in bottleneck_data:
            ...
            if memory_mb > 100:  # High memory usage
                root_causes.append(
                    "High memory usage - potential memory leaks or inefficient data structures"
                )
    ...
    def _generate_recommendations(
        ...
        if event_type in [
            ...
            recommendations.extend(
                self._get_database_recommendations(operation_name, event_type, bottleneck_data)
            )
        ...
        if event_type in [
            ProfilerEvent.CACHE_GET,
            ProfilerEvent.CACHE_SET,
            ProfilerEvent.CACHE_DELETE,
        ]:
            recommendations.extend(
                self._get_cache_recommendations(operation_name, event_type, bottleneck_data)
            )
        ...
        if event_type == ProfilerEvent.CONSISTENCY_CHECK:
            recommendations.extend(
                self._get_consistency_recommendations(operation_name, bottleneck_data)
            )

        # Memory-related recommendations
        if "High memory usage" in root_causes:
            recommendations.extend(
                self._get_memory_recommendations(operation_name, bottleneck_data)
            )

        return recommendations

    def _get_database_recommendations(
        self, operation_name: str, event_type: ProfilerEvent, bottleneck_data: dict[str, Any]
    ) -> list[OptimizationRecommendation]:
    ...
    def _get_sync_recommendations(
        self, operation_name: str, bottleneck_data: dict[str, Any]
    ) -> list[OptimizationRecommendation]:
