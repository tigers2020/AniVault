...
class ParallelPipelineManager:
    ...
    def execute_pipeline(self) -> dict[PipelineStage, StageResult]:
        ...
        try:
            ...
            with self.executor_manager.get_general_executor() as executor:
                ...
                while (
                    len(self.completed_stages) < len(self.task_definitions) and not self._cancelled
                ):
    ...
    def _process_completed_tasks_with_collector(
        self, executor: ThreadPoolExecutor, result_collector: ResultCollector
    ) -> None:
        """Process completed tasks with enhanced result collection.
            ...
            if stage_result:
                ...
                if stage_result.success:
                    if stage_result.has_partial_success:
                        ...
                        logger.info(
                            f"Stage {stage} completed successfully: {stage_result.total_tasks} tasks"
                        )
                else:
                    logger.warning(
                        f"Stage {stage} failed: {stage_result.failed_tasks} tasks failed"
                    )
    ...
    def get_progress(self) -> dict[str, Any]:
        ...
        return {
            ...
            "progress_percentage": (
                (completed_count / total_stages) * 100 if total_stages > 0 else 0
            ),
