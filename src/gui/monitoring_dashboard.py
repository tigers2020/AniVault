...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QGroupBox, QPushButton, QTextEdit, QScrollArea
)
...
from ..core.sync_monitoring import sync_monitor, SyncOperationType
    ...
    def update_metrics(self):
        ...
        for op_type, type_stats in stats.operation_type_stats.items():
            if type_stats['count'] > 0:
                ...
                self.operation_table.setItem(row, 4, QTableWidgetItem(str(type_stats['total_affected_records'])))
                row += 1
...
class CacheMetricsWidget(MetricsWidget):
    ...
    def setup_cache_metrics(self):
        ...
        self.events_table.setHorizontalHeaderLabels([
            "Event Type", "Operation Type", "Count"
        ])
...
"""
            ...
            for op in recent_ops:
                summary += f"{op.operation_type.value}: {op.status.value} ({op.duration_ms:.2f}ms)\n"
...
class MonitoringDashboard(QWidget):
    ...
    def update_all_metrics(self):
        ...
        try:
            ...
            self.status_label.setText("Monitoring Active - Last Updated: " + time.strftime("%H:%M:%S"))
            self.status_label.setStyleSheet("color: green; font-weight: bold;")
