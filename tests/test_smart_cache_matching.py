...
from src.core.smart_cache_matcher import QueryNormalizer, SimilarityDetector, SmartCacheMatcher
...
class TestQueryNormalizer(unittest.TestCase):
    ...
    def test_stop_words_removal(self) -> None:
...
class TestSimilarityDetector(unittest.TestCase):
    ...
    def test_exact_match(self) -> None:
        ...
        matches = self.detector.find_similar_queries(
            target, candidates, use_fuzzy=True, use_phonetic=False
        )
    ...
    def test_fuzzy_matching(self) -> None:
        ...
        for match in matches:
            assert match.similarity_score >= 60.0
    ...
    def test_empty_inputs(self) -> None:
        ...
        # Empty target
        matches = self.detector.find_similar_queries(
            "", ["attack on titan"], use_fuzzy=True, use_phonetic=False
        )
        assert len(matches) == 0
        ...
        # None inputs
        matches = self.detector.find_similar_queries(
            None, ["attack on titan"], use_fuzzy=True, use_phonetic=False
        )
        assert len(matches) == 0

    def test_match_sorting(self) -> None:
...
class TestSmartCacheMatcher(unittest.TestCase):
    ...
    def test_find_similar_cache_keys(self) -> None:
        ...
        similar_keys = self.matcher.find_similar_cache_keys(
            "attack on titan", cache_keys, use_fuzzy=True, use_phonetic=False
        )
        assert len(similar_keys) > 0

        # Test with different query
        similar_keys = self.matcher.find_similar_cache_keys(
            "one piece", cache_keys, use_fuzzy=True, use_phonetic=False
        )
        assert len(similar_keys) > 0

    def test_generate_similarity_keys(self) -> None:
        ...
        assert "attack titan" in keys

    def test_should_use_smart_matching(self) -> None:
        """Test decision logic for using smart matching."""
        # Test with queries that should use smart matching
        assert self.matcher.should_use_smart_matching("Attack on Titan: The Final Season")
        assert self.matcher.should_use_smart_matching("Attack on Titan (2023)")

        # Should not use smart matching for very short queries
        assert not self.matcher.should_use_smart_matching("A")
        assert not self.matcher.should_use_smart_matching("An")
        assert not self.matcher.should_use_smart_matching(
            "Attack on Titan"
        )  # No variation patterns
...
class TestSmartCacheMatchingIntegration(unittest.TestCase):
    ...
    def test_smart_get_basic(self) -> None:
        ...
        anime2 = TMDBAnime(
            ...
            first_air_date=datetime(2020, 12, 7),
        )
    ...
    def test_smart_get_with_different_queries(self) -> None:
    ...
    def test_smart_get_no_matches(self) -> None:
        ...
        results = self.cache.get_smart(
            "completely different anime", similarity_threshold=0.6, max_results=5
        )
        assert len(results) == 0

    def test_smart_get_threshold_filtering(self) -> None:
        ...
        results_high = self.cache.get_smart(
            "attack on titan", similarity_threshold=0.9, max_results=5
        )

        # Test with low threshold (should find more matches)
        results_low = self.cache.get_smart(
            "attack on titan", similarity_threshold=0.3, max_results=5
        )

        assert len(results_high) <= len(results_low)

    def test_smart_get_max_results_limit(self) -> None:
        """Test that max_results parameter limits the number of results."""
        # Store multiple test data
        for i in range(10):
            anime = TMDBAnime(
                tmdb_id=i,
                title=f"Attack on Titan Season {i}",
                first_air_date=datetime(2013 + i, 4, 7),
            )
    ...
    def test_similarity_key_storage(self) -> None:
        ...
        assert stats["total_similarity_keys"] > 0

    def test_find_similar_keys(self) -> None:
        ...
        assert "attack_on_titan_2013" in key_names

    def test_similarity_stats(self) -> None:
        ...
        expected_fields = [
            "total_cache_keys",
            "total_similarity_keys",
            "average_similarity_keys_per_entry",
            "similarity_key_coverage",
        ]
        ...
        assert stats["total_similarity_keys"] > 0
        assert stats["average_similarity_keys_per_entry"] > 0
        assert stats["similarity_key_coverage"] >= 0.0
        assert stats["similarity_key_coverage"] <= 1.0
...
class TestSmartCacheMatchingPerformance(unittest.TestCase):
    ...
    def test_smart_get_performance(self) -> None:
        """Test performance of smart get operations."""
        # Store large dataset
        for i in range(100):
            anime = TMDBAnime(
                tmdb_id=i, title=f"Anime {i}", first_air_date=datetime(2000 + i, 1, 1)
            )
    ...
    def test_similarity_key_generation_performance(self) -> None:
        ...
        start_time = time.time()
        for i in range(100):
            matcher.generate_similarity_keys(f"Anime {i}")
        end_time = time.time()
    ...
    def test_cache_hit_rate_improvement(self) -> None:
        ...
        smart_results = self.cache.get_smart(
            "attack on titan", similarity_threshold=0.6, max_results=5
        )
...
class TestSmartCacheMatchingEdgeCases(unittest.TestCase):
    ...
    def test_smart_get_with_zero_max_results(self) -> None:
    ...
    def test_smart_get_with_negative_threshold(self) -> None:
