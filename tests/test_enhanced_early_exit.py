...
from src.core.tmdb_client import TMDBClient, TMDBConfig, SearchStrategyType
...
class TestEnhancedEarlyExit(unittest.TestCase):
    ...
    def test_progressive_threshold_early_exit(self):
        ...
        with patch.object(self.client, '_strategy_exact_title_with_year') as mock_strategy1, \
            ...
            self.assertEqual(results[0].quality_score, 0.9)
    ...
    def test_medium_quality_early_exit(self):
        ...
        with patch.object(self.client, '_strategy_exact_title_with_year') as mock_strategy1, \
            ...
            self.assertEqual(len(results), 1)
            self.assertEqual(results[0].quality_score, 0.8)
            self.assertFalse(needs_selection)
    ...
    def test_low_quality_early_exit(self):
        ...
        with patch.object(self.client, '_strategy_exact_title_with_year') as mock_strategy1, \
            ...
            self.assertEqual(len(results), 1)
            self.assertEqual(results[0].quality_score, 0.7)
            self.assertFalse(needs_selection)
    ...
    def test_multiple_high_quality_results_early_exit(self):
        ...
        with patch.object(self.client, '_strategy_exact_title_with_year') as mock_strategy1, \
            ...
            self.assertTrue(needs_selection)  # Multiple results need selection
    ...
    def test_no_early_exit_with_low_quality(self):
        ...
        with patch.object(self.client, '_strategy_exact_title_with_year') as mock_strategy1, \
            ...
            self.assertEqual(len(results), 2)
            self.assertTrue(needs_selection)
    ...
    def test_early_exit_with_reasonable_results(self):
        ...
        with patch.object(self.client, '_strategy_exact_title_with_year') as mock_strategy1, \
            ...
            self.assertEqual(len(results), 1)
            self.assertEqual(results[0].quality_score, 0.68)
            self.assertFalse(needs_selection)
    ...
    def test_early_exit_threshold_configuration(self):
        ...
        for strategy_type, expected_threshold in strategies:
            ...
            if strategy_type == SearchStrategyType.EXACT_TITLE_WITH_YEAR:
                self.assertEqual(expected_threshold, 0.85)
            elif strategy_type == SearchStrategyType.EXACT_TITLE_ONLY:
                self.assertEqual(expected_threshold, 0.75)
            elif strategy_type == SearchStrategyType.CLEANED_TITLE:
                self.assertEqual(expected_threshold, 0.65)
    ...
    def test_early_exit_logging(self):
        ...
        with patch.object(self.client, '_strategy_exact_title_with_year') as mock_strategy1, \
            ...
            self.assertTrue(early_exit_logged)
