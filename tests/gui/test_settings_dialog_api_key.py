...
def test_save_api_key_to_env_file_new(temp_env_file: Path):
    """Test saving API key to new .env file."""
    # Setup
    with patch("pathlib.Path") as mock_path:
        mock_path.return_value = temp_env_file.parent
        ...
        dialog._save_api_key_to_env_file(test_key)
...
def test_save_api_key_to_env_file_update_existing(temp_env_file: Path):
    ...
    with patch("pathlib.Path") as mock_path:
        ...
        dialog._save_api_key_to_env_file(new_key)
...
def test_save_api_key_to_env_file_preserves_comments(temp_env_file: Path):
