...
class TestParallelMetadataRetrieval(unittest.TestCase):
    ...
    def test_parallel_metadata_retrieval(self, mock_tmdb_client_class) -> None:
        ...
        for file in result:
            assert file.tmdb_info is not None
            assert file.tmdb_info.tmdb_id == 12345
            assert file.tmdb_info.title == "Test Anime"

        # Verify that TMDB client was called for each file
        assert mock_tmdb_client.search_tv_series.call_count == 5

    @patch("src.core.services.file_processing_tasks.TMDBClient")
    def test_parallel_metadata_retrieval_with_errors(self, mock_tmdb_client_class) -> None:
        ...
        for file in result:
            if file.tmdb_info is not None:
                ...
                error_count += 1
                assert any("Metadata retrieval failed" in error for error in file.processing_errors)

        # Should have 2 successes and 3 errors
        assert success_count == 2
        assert error_count == 3

    def test_thread_executor_manager_integration(self) -> None:
        """Test that the task integrates correctly with ThreadExecutorManager."""
        # Get the thread executor manager
        manager = get_thread_executor_manager()

        # Verify it has the expected methods
        assert hasattr(manager, "get_tmdb_executor")
        assert hasattr(manager, "get_file_scan_executor")
        assert hasattr(manager, "get_general_executor")

        # Test that executors are created correctly
        tmdb_executor = manager.get_tmdb_executor()
        assert isinstance(tmdb_executor, ThreadPoolExecutor)
        ...
        assert config["tmdb_max_workers"] > 0
        assert config["tmdb_max_workers"] <= 100  # Reasonable upper bound
