...
class TestFileGrouperPerformance:
    ...
    def _generate_test_files(self, count: int, title_variations: int = 5) -> List[ScannedFile]:
    ...
    def test_performance_small_dataset(self) -> None:
        ...
        assert execution_time < 1.0, f"Small dataset took too long: {execution_time:.3f}s"
    ...
    def test_performance_medium_dataset(self) -> None:
        ...
        assert execution_time < 5.0, f"Medium dataset took too long: {execution_time:.3f}s"
        assert len(result) <= 25, "Should group similar files together"
        
        print(f"Medium dataset (100 files): {execution_time:.3f}s, {len(result)} groups")
    ...
    def test_performance_large_dataset(self) -> None:
        ...
        assert execution_time < 30.0, f"Large dataset took too long: {execution_time:.3f}s"
    ...
    def test_performance_scalability(self) -> None:
        ...
        if len(times) >= 2:
            ...
            assert ratio_100_200 < 4.0, f"Performance ratio too high: {ratio_100_200:.2f}"
            
            print(f"Performance ratios: 50->100: {ratio_50_100:.2f}, 100->200: {ratio_100_200:.2f}")
    ...
    def test_hash_based_grouping_efficiency(self) -> None:
        ...
        try:
            ...
            assert similarity_call_count < total_possible_comparisons, \
            ...
            print(f"Similarity calculations: {similarity_call_count}/{total_possible_comparisons} ({reduction_ratio:.2%})")
    ...
    def test_union_find_efficiency(self) -> None:
        ...
        assert len(aot_files) == 3, f"Expected 3 Attack on Titan files, got {len(aot_files)}"
