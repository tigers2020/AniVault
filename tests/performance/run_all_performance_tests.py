...
class PerformanceTestRunner:
    ...
    def __init__(self) -> None:
        ...
        Sets up internal data structures for tracking test results and timing.
        """
    ...
    def generate_unified_report(self) -> dict[str, Any]:
        ...
        completed_tests = sum(
            1 for result in self.test_results.values() if result["status"] == "completed"
        )
        ...
        if "bulk_update_performance" in self.test_results:
            ...
            if bulk_test["status"] == "completed" and "summary" in bulk_test["results"]:
                ...
                metrics["bulk_update_performance"] = {
                    "anime_avg_execution_time": summary.get("anime_metadata_summary", {}).get(
                        "avg_execution_time", 0
                    ),
                    "file_avg_execution_time": summary.get("parsed_files_summary", {}).get(
                        "avg_execution_time", 0
                    ),
                    "avg_time_improvement": summary.get("overall_improvements", {}).get(
                        "avg_time_improvement_percent", 0
                    ),
                    "avg_query_reduction": summary.get("overall_improvements", {}).get(
                        "avg_query_reduction_percent", 0
                    ),
                }
        ...
        if "n1_query_elimination" in self.test_results:
            ...
            if n1_test["status"] == "completed" and "summary" in n1_test["results"]:
                ...
                metrics["n1_elimination_metrics"] = {
                    "elimination_rate_percent": summary.get("efficiency_metrics", {}).get(
                        "n1_elimination_rate", 0
                    ),
                    "avg_queries_per_record": summary.get("average_queries_per_record", 0),
                    "total_queries_analyzed": summary.get("efficiency_metrics", {}).get(
        ...
        if "memory_and_speed_benchmarks" in self.test_results:
            ...
            if benchmark_test["status"] == "completed" and "summary" in benchmark_test["results"]:
                ...
                metrics["memory_efficiency"] = {
                    "anime_avg_memory_per_record": summary.get("memory_efficiency", {}).get(
                        "anime_avg_memory_per_record_mb", 0
                    ),
                    "file_avg_memory_per_record": summary.get("memory_efficiency", {}).get(
                        "file_avg_memory_per_record_mb", 0
                    ),
                }
                metrics["throughput_performance"] = {
                    "avg_throughput_records_per_sec": summary.get("throughput_performance", {}).get(
                        "avg_throughput_records_per_sec", 0
                    ),
                    "max_throughput_records_per_sec": summary.get("throughput_performance", {}).get(
                        "max_throughput_records_per_sec", 0
                    ),
                }
                metrics["scalability_metrics"] = {
                    "is_linearly_scalable": summary.get("scaling_characteristics", {}).get(
                        "is_linearly_scalable", False
                    ),
                    "scaling_efficiency": summary.get("scaling_characteristics", {}).get(
        ...
        if "large_dataset_batch_processing" in self.test_results:
            ...
            if large_test["status"] == "completed" and "summary" in large_test["results"]:
                ...
                if "performance_characteristics" in summary:
                    metrics["throughput_performance"]["optimal_batch_size"] = summary[
                        "performance_characteristics"
                    ].get("optimal_batch_size", 0)
                    metrics["throughput_performance"]["best_throughput"] = summary[
                        "performance_characteristics"
                    ].get("best_throughput_records_per_sec", 0)
    ...
    def validate_optimizations(self) -> dict[str, Any]:
        ...
        if bulk_metrics:
            ...
            if time_improvement >= criteria["min_time_improvement_percent"]:
                validation["passed_validations"].append(
                    f"Time improvement: {time_improvement:.1f}% >= {criteria['min_time_improvement_percent']}%"
                )
            else:
            ...
            if query_reduction >= criteria["min_query_reduction_percent"]:
                validation["passed_validations"].append(
                    f"Query reduction: {query_reduction:.1f}% >= {criteria['min_query_reduction_percent']}%"
                )
            else:
                validation["failed_validations"].append(
                    f"Query reduction: {query_reduction:.1f}% < {criteria['min_query_reduction_percent']}%"
        ...
        if n1_metrics:
            ...
            if queries_per_record <= criteria["max_queries_per_record"]:
                validation["passed_validations"].append(
                    f"Queries per record: {queries_per_record:.4f} <= {criteria['max_queries_per_record']}"
                )
            else:
                validation["failed_validations"].append(
                    f"Queries per record: {queries_per_record:.4f} > {criteria['max_queries_per_record']}"
        ...
        if throughput_metrics:
            ...
            if avg_throughput >= criteria["min_throughput_records_per_sec"]:
                validation["passed_validations"].append(
                    f"Average throughput: {avg_throughput:.1f} >= {criteria['min_throughput_records_per_sec']} records/sec"
                )
            else:
                validation["failed_validations"].append(
                    f"Average throughput: {avg_throughput:.1f} < {criteria['min_throughput_records_per_sec']} records/sec"
    ...
    def save_unified_report(self, report: dict[str, Any], filename: str | None = None) -> None:
    ...
    def print_summary_report(self, report: dict[str, Any]) -> None:
        ...
        if metrics.get("throughput_performance"):
            ...
            logger.info(
                f"  Optimal Batch Size: {throughput.get('optimal_batch_size', 0):,} records"
            )
        ...
        logger.info(
            f"  All Optimizations Validated: {'✅' if validation['all_optimizations_validated'] else '❌'}"
        )
